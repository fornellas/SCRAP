 ...
 *
 *  Copyright (C) 2005 Fabio Pugliese Ornellas <fabio.ornellas@gmail.com>
 *                     - Added support for multi-thread AVI_errno (POSIX only)
 *                     - Create an AVI header after adding tracks, so that
 *                       it can be readable if not closed.
 *                     - Video codec keyframe setting
 *                     - Misc bug fixes
...
/**
 * Cygwin and DJGPP need the O_BINARY flag, which
 * doesn't exist on most Unix systems)
 */
#elif __CYGWIN__ || __DJGPP__
#include <errno.h>
#include <sys/types.h>
#include <unistd.h>
#define OPEN_RDWR (O_RDWR | O_BINARY)
#define OPEN_CREAT (O_CREAT | O_BINARY)
...
/*
    AVI_errno
    The next functions are responsable for thread specific AVI_errno.
*/
#ifdef MULTI_THREAD
static pthread_once_t AVI_errno_key_once = PTHREAD_ONCE_INIT;
pthread_key_t AVI_errno_key;

/* Free the thread-specific AVI_errno */
static void AVI_errno_destroy(void * buf)
{
  free(buf);
}

/* Allocate the key */
static void AVI_errno_key_alloc(void)
{
  if(pthread_key_create(&AVI_errno_key, AVI_errno_destroy)==EAGAIN)
    exit(1); /* PTHREAD_KEYS_MAX reached */
}

/* Allocate the thread-specific AVI_errno */
void AVI_errno_alloc(void)
{
  long *p;
  pthread_once(&AVI_errno_key_once, AVI_errno_key_alloc);
  if(pthread_getspecific(AVI_errno_key)==NULL) {
    if(pthread_setspecific(AVI_errno_key, malloc(sizeof(long)))==EINVAL)
      exit(1); /* Weird, no such key... */
    if((p=(long *)pthread_getspecific(AVI_errno_key))==NULL)
      exit(1); /* Not enought memory */
    (*p)=0;
  }
}

/* Return the thread-specific AVI_errno */
long * get_AVI_errno(void)
{
  return (long *) pthread_getspecific(AVI_errno_key);
}
#endif
...
/*
   ...
   returns a pointer to avi_t on success, a NULL pointer on error
*/
...
avi_t* AVI_open_output_file(char * filename)
{
   ...
   errno = 0;
   AVI->fdes = open(filename,OPEN_RDWR|OPEN_CREAT, 0666);
   if (AVI->fdes < 0)
   {
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   OUTSHRT((int)(AVI->a_fmt));           /* Format */
   OUTSHRT((int)(AVI->a_chans));         /* Number of channels */
   OUTLONG((int)(AVI->a_rate));          /* SamplesPerSec */
   OUTLONG(sampsize*AVI->a_rate); /* AvgBytesPerSec */
   OUTSHRT(sampsize);             /* BlockAlign */
   OUTSHRT((int)(AVI->a_bits));          /* BitsPerSample */
...
static int avi_write_data(avi_t *AVI, char *data, long length, int audio, int keyframe)
{
   int n;
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   if(-1==lseek(AVI->fdes,AVI->pos,SEEK_SET)) {
      AVI_errno = AVI_ERR_WRITE;
      return -1;
   }
...
avi_t *AVI_open_input_file(const char *filename, int getIndex)
{
   ...
   AVI->audio_tag[0] = (char)(AVI->audio_strn/10 + '0');
   AVI->audio_tag[1] = (char)(AVI->audio_strn%10 + '0');
   AVI->audio_tag[2] = 'w';
   AVI->audio_tag[3] = 'b';
   ...
   if(AVI->audio_chunks)
   {
      ...
      if(AVI->audio_index==NULL) ERR_EXIT(AVI_ERR_NO_MEM)
   }
...
#ifdef MULTI_THREAD
/*
  Thread specific error_string.
*/
static pthread_once_t error_string_key_once = PTHREAD_ONCE_INIT;
pthread_key_t error_string_key;

/* Free the thread-specific error_string */
static void error_string_destroy(void * buf)
{
  free(buf);
}

/* Allocate the key */
static void error_string_key_alloc(void)
{
  if(pthread_key_create(&error_string_key, error_string_destroy)==EAGAIN)
    exit(1); /* PTHREAD_KEYS_MAX reached */
}
#else
...
char *AVI_strerror()
{
   ...
   pthread_once(&error_string_key_once, error_string_key_alloc);
   ...
   if(AVI_errno == AVI_ERR_OPEN ||
      ...
      AVI_errno == AVI_ERR_CLOSE )
   {
      asprintf(&p,"%s - %s",avi_errors[aerrno],
#ifndef _WIN32
      ...
      free(pthread_getspecific(error_string_key));
      pthread_setspecific(error_string_key, (void *)p);
      return p;
   }
   else
   {
